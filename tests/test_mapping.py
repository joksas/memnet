...
def test_double_w_to_G(args, expected):
    ...
    G, max_weight = mapping.double_w_to_G(**args)
...
def test_ideal_dpe(args, expected, G_off, G_on, k_V):
    ...
    V = mapping.x_to_V(x, k_V)

    G, max_weight = mapping.w_to_G(w, G_off, G_on)

    I = nonidealities.LinearityPreserving.compute_I(V, G)
